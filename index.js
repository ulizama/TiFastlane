...
import tiappxml from "tiapp.xml";
...
const _exec = execChild;

let cfgfile = "tifastlane.cfg";
let infile = "tiapp.xml";
let certDir = "./TiFLCertificates";
let deliveryDir = "./TiFLDelivery";
let pilotDir = "./TiFLPilot";
let tiapp = {};
let TiVersion = {};
let cfg = {
  cli: "appc",
...
let appDeliveryDir = null;
let appAndroidDeliveryDir = null;
let deliverFile = null;
let appDeliveryMetaDir = null;
let appDeliveryScreenDir = null;
let fastlaneBinary = "fastlane";
let canLoad = true;

/*
@ Read configuration files
*/
export function loadconfig(cfg_file) {
  if (cfg_file) {
    cfgfile = cfg_file;
  ...
  if (!fs.existsSync(infile)) {
    ...
    console.log(
      chalk.yellow("tifast must be run on the root of your Titanium App"),
    );
    process.exit();
  }
  ...
  if (!fs.existsSync(cfgfile)) {
    ...
    console.log(
      chalk.yellow("Run ") +
        chalk.cyan("tifast setup") +
        chalk.yellow(" to initialize your tifast configuration."),
    );
    process.exit();
  }
  ...
  ((appDeliveryDir = deliveryDir + "/" + tiapp.id),
    ...
    (appDeliveryMetaDir = !cfg.locale
      ? appDeliveryDir + "/metadata/en-US"
      : appDeliveryDir + "/metadata/" + cfg.locale),
    (appDeliveryScreenDir = !cfg.locale
      ? appDeliveryDir + "/screenshots/en-US"
      : appDeliveryDir + "/screenshots/" + cfg.locale),
    (fastlaneBinary = cfg.fastlane_binary
      ? path.isAbsolute(cfg.fastlane_binary) ||
        cfg.fastlane_binary === "fastlane"
        ? cfg.fastlane_binary
...
function bumpBundleVersion() {
  ...
  tiapp = tiapp.replace(
    /(<key>CFBundleVersion<\/key>\s*<string>)([^< ]+)(<\/string>)/gm,
    function (match, before, CFBundleVersion, after) {
...
function localStatus(e) {
  ...
  _tiappFile.replace(
    /(versionCode=\")([^< ]+)(")/gm,
    function (match, before, _versionCode, after) {
  ...
  _tiappFile.replace(
    /(<key>CFBundleVersion<\/key>\s*<string>)([^< ]+)(<\/string>)/gm,
    function (match, before, _CFBundleVersion, after) {
  ...
  console.log(
    "SDK Version:",
    "major",
    chalk.cyan(TiVersion.major),
    "minor",
    chalk.cyan(TiVersion.minor),
    "patch",
    chalk.cyan(TiVersion.patch),
  );

  if (e.type !== "Android") {
    console.log(chalk.cyan("iOS:"));
    if (cfg.apple_id != "null")
      console.log("\t", "Apple ID:", chalk.cyan(cfg.apple_id));
    if (cfg.team_id != "null")
      console.log("\t", "Team ID:", chalk.cyan(cfg.team_id));
    if (cfg.team_name != "null")
      console.log("\t", "Team Name:", chalk.cyan(cfg.team_name));
  ...
  if (e.type !== "iOS") {
    ...
    if (cfg.android_app_id != "null")
      console.log("\t", "Android AppID:", chalk.cyan(cfg.android_app_id));
...
function uploadBetaTestIPA(opts) {
  ...
  if (opts.skip_build) {
    ...
    console.log(
      chalk.yellow("First things first. Clean project to ensure build"),
    );
    console.log("\n");
...
function smartInit() {
  ...
  if (fs.existsSync(appDeliveryDir + "/Deliverfile")) {
    console.log(
      chalk.red(
        'App is already initialized. If you wish to redo the process please remove the "' +
          appDeliveryDir +
          '" directory',
      ),
    );
  ...
  exec(fastlaneBinary, initArgs, { cwd: appDeliveryDir }, function (e) {
    ...
    console.log(
      chalk.green(
        "You can find your configuration files for delivery on: " +
          appDeliveryDir,
      ),
    );
...
function dealWithResults(json) {
  ...
  cfg.google_play_json_key = json.google_play_json_key
    ? json.google_play_json_key
    : null;
  cfg.google_play_issuer = json.google_play_issuer
    ? json.google_play_issuer
    : null;
  cfg.google_keystore_file = json.google_keystore_file
    ? json.google_keystore_file
    : null;
  cfg.google_keystore_password = json.google_keystore_password
    ? json.google_keystore_password
    : null;
  cfg.google_keystore_alias = json.google_keystore_alias
    ? json.google_keystore_alias
    : null;
  cfg.fastlane_binary = json.fastlane_binary
    ? json.fastlane_binary
    : "fastlane";
  cfg.ios_build_args = json.ios_build_args ? json.ios_build_args : null;
  cfg.android_build_args = json.android_build_args
    ? json.android_build_args
    : null;
  ...
  cfgFile = cfgFile
    ...
    .replace("[APPLE_ID]", cfg.apple_id)
    .replace("[TEAM_ID]", cfg.team_id)
    .replace("[TEAM_NAME]", cfg.team_name)
    .replace("[GOOGLE_PLAY_JSON_KEY]", cfg.google_play_json_key)
  ...
  inquirer
    .prompt([
...

      {
        ...
        message:
          "Keystore Location. Path to your keystore file that is used to sign your application.",
      },
...

      {
        ...
        name: "android_app_id",
        message:
          "Optional Application ID to use for Android, if empty it will use the default one",
      },

      {
        ...
        message:
          "Optional absolute path or relative to this config, to the Fastlane binary",
        default: "fastlane",
      },
    ])
...
export function init(opts) {
  ...
  if (opts.smart) {
    ...
    _deliverFile = _deliverFile
      .replace("[APP_ID]", tiapp.id)
      .replace("[EMAIL]", cfg.apple_id);
    ...
    console.log(
      chalk.green('Your app "deliver" configuration has been created.'),
    );
    console.log(
      chalk.green(
        "You can find your configuration files on: " + appDeliveryDir,
      ),
    );
...
export function send(opts) {
  ...
  if (cfg.ios_build_args && cfg.ios_build_args != "") {
    ...
    if (regex.test(cfg.ios_build_args)) {
      ...
      console.log(
        chalk.red(
          ' Starting TiFastlane v0.9 the "ios_build_args" configuration changed how it behaves.',
        ),
      );
      console.log(
        chalk.red(
          " Now you use it to append build args instead of replacing them",
        ),
      );
      console.log(
        chalk.red(
          " You cannot use -p (platform), -T (target) or -O (output) when appending new args",
        ),
      );
      console.log(chalk.red("==================================="));
      console.log("\n ");
  ...
  function _deliver(sendipa) {
    ...
    if (sendipa) {
      ...
      initArgs.push("--skip_binary_upload");
    }
    ...
    if (opts.skip_screenshots) {
      initArgs.push("--skip_screenshots");
    }
    ...
    if (opts.skip_waiting_for_build_processing) {
      initArgs.push("--skip_waiting_for_build_processing");
    }

    if (opts.automatic_release) {
      initArgs.push("--automatic_release");
    }
    ...
    fs.readFileSync(deliverFile)
      ...
      .forEach(function (line) {
        if (/^version /.test(line)) {
          //Update Version
          newFileContents =
            newFileContents + 'version "' + tiapp.version + '"' + "\n";
        }
...
export function register(opts) {
  ...
  exec(fastlaneBinary, produceArgs, null, function (e) {
    ...
    platforms.forEach(function (p) {
      if (opts.match) {
        ...
        var matchArgs = ["match", p, "-u", cfg.apple_id, "-a", tiapp.id];
        ...
        exec(fastlaneBinary, matchArgs, null, function (e) {
          //Done
        });
      } else {
        console.log(
          chalk.cyan("Creating Provision Profile on environment: " + p),
        );
        ...
        exec(fastlaneBinary, sighArgs, null, function (e) {
          //Done
        });
      }
    });
...
*/
export function repairprofiles(opts) {
  if (!fs.existsSync(cfgfile)) {
    console.log(chalk.red("==================================="));
    console.log(chalk.red("Cannot find ", cfgfile));
    console.log(chalk.yellow("You must run ") + chalk.cyan("tifast setup"));
...
*/
export function downloadprofiles(opts) {
  if (!fs.existsSync(cfgfile)) {
    console.log(chalk.red("==================================="));
    console.log(chalk.red("Cannot find ", cfgfile));
    console.log(chalk.yellow("You must run ") + chalk.cyan("tifast setup"));
...
export function pem(opts) {
  ...
  var pemArgs = ["pem", "-e", certDir, "-a", tiapp.id, "-u", cfg.apple_id];
...
function bumpBundleVersionAndroid() {
  ...
  tiapp = tiapp.replace(
    /(versionCode=\")([^< ]+)(")/gm,
    function (match, before, versionCode, after) {
...
function manageAppID(osType) {
  ...
  // clean org.id
  var _orgId;
  tiappXml = tiappXml.replace(
    /(<org.id>)([^< ]+)(<\/org\.id>)/gm,
    function (match, before, orgId, after) {
      if (!_orgId) _orgId = orgId;
      return before + after;
    },
  );
  tiappXml = tiappXml.replace(/(<org.id><\/org\.id>)/gm, "");

  if (osType == "android" && cfg.android_app_id) {
    tiappXml = tiappXml.replace(
      /(<id>)([^< ]+)(<\/id>)/gm,
      function (match, before, appId, after) {
        console.log(
          chalk.green("Refine Application ID to: " + cfg.android_app_id),
        );
...
export function playinit(opts) {
  ...
  var initArgs = [
    ...
    "init",
    "--json_key",
    "../../../" + cfg.google_play_json_key,
    "--package_name",
    cfg.android_app_id != "null" ? cfg.android_app_id : tiapp.id,
  ];
...
export function playsend(opts) {
  ...
  if (cfg.android_build_args && cfg.android_build_args != "") {
    ...
    if (regex.test(cfg.android_build_args)) {
      ...
      console.log(
        chalk.red(
          ' Starting TiFastlane v0.9 the "android_build_args" configuration changed how it behaves.',
        ),
      );
      console.log(
        chalk.red(
  ...
  function _supply(sendapk) {
    ...
    if (sendapk) {
      if (opts.track) {
        initArgs.push("--track", opts.track);
      }

      if (opts.rollout) {
        initArgs.push("--rollout", opts.rollout);
      }

      if (opts.aab) {
        initArgs.push("--aab", "../../../dist/" + tiapp.name + ".aab");
      } else {
        initArgs.push("--apk", "../../../dist/" + tiapp.name + ".apk");
      }
    } else {
      initArgs.push("--skip_upload_apk");
    }

    if (opts.skip_upload_graphic_assets) {
      initArgs.push("--skip_upload_images", "--skip_upload_screenshots");
    }

    if (opts.skip_upload_images) {
      initArgs.push("--skip_upload_images");
    }

    if (opts.validate_only) {
      initArgs.push("--validate_only");
    }

    if (opts.skip_upload_screenshots) {
      initArgs.push("--skip_upload_screenshots");
    }

    if (opts.skip_upload_metadata) {
      initArgs.push("--skip_upload_metadata");
    }

    exec(
      fastlaneBinary,
      initArgs,
      { cwd: appAndroidDeliveryDir },
      function (e) {
        console.log(chalk.green("\nSupply Done\n"));
      },
    );
  ...
  if (opts.skip_upload_apk) {
    ...
    if (opts.skip_build) {
      ...
      exec(cfg.cli, cleanArgs, null, function (e) {
        ...
        buildArgs.push(
          "-K",
          cfg.google_keystore_file,
          "-P",
          cfg.google_keystore_password,
          "-L",
          cfg.google_keystore_alias,
        );
...
function buildIPA(opts, callback) {
  ...
  if (opts.legacy) {
    console.log(
      chalk.yellow(
        "------- Enabling legacy mode for app build and packaging -------",
      ),
    );
    console.log(
      chalk.yellow(
        "This mode only works if you are building with Titanium CLI < 6.0.2 and XCODE < 8.3",
      ),
    );
    console.log(
      chalk.yellow("If the build fails please disable the legacy mode"),
    );
    console.log(
      chalk.yellow(
        "----------------------------------------------------------------",
  ...
  if (opts.distribution_name) {
    buildArgs.push("-R", opts.distribution_name);
  }

  if (opts.pp_uuid) {
    buildArgs.push("-P", opts.pp_uuid);
  }

  if (
    cfg.ios_build_args &&
    cfg.ios_build_args != "" &&
    cfg.ios_build_args != "null"
  ) {
  ...
  if (TiVersion.major <= 5 || (TiVersion.major === 6 && TiVersion.minor < 1)) {
    exec(cfg.cli, buildArgs, null, function (e) {
      if (opts.legacy) {
        ...
        console.log(
          chalk.yellow(
            "If you use Xcode 9, please run 'tifast send' command with --pp_uuid option. If not, building IPA file will be failed.",
          ),
        );
        ...
        var exporterArgs = [
          ...
          "./dist",
          "-exportOptionsPlist",
          "./build/iphone/build_exporter.plist",
        ];
    ...
    buildArgs.push("--export-ipa");

    buildArgs.push("--output-dir", "./dist");
