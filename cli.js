...
import * as tifastlane from "./index.js";
...
program
  ...
  .option(
    "-s, --smart",
    "If your app is already on the App Store run init with -s",
  )
  .action(init);
...
program
  ...
  .option(
    "--submit_for_review",
    "Submit the new version for Review after uploading everything (DELIVER_SUBMIT_FOR_REVIEW)",
  )
  .option(
    "--skip_waiting_for_build_processing",
    "Don't wait for the build to process. If set to true, the changelog won't be set (PILOT_SKIP_WAITING_FOR_BUILD_PROCESSING)",
  )
  .option(
    "--automatic_release",
...
program
  ...
  .option(
    "-sp, --skip_profiles",
    "[sigh] Skip the creation of Provisioning Profiles",
  )
  .option(
    "-si, --skip_install",
    "[sigh] Skip installation of new provisioning profiles",
  )
  .option(
    "-sf, --skip_fetch_profiles",
    "[sigh] Skips the verification of existing profiles which is useful if you have thousands of profiles",
  )
  .action(register);
...
program
  ...
  .option(
    "-d, --development",
    "Renew the development push certificate instead of the production one (PEM_DEVELOPMENT)",
  )
  .option(
    "-g, --generate_p12",
    "Generate a p12 file additionally to a PEM file (PEM_GENERATE_P12_FILE)",
  )
  .option(
    "-s, --save_private_key",
    "Set to save the private RSA key (PEM_SAVE_PRIVATEKEY)",
  )
  .option(
    "-f, --force",
    "Create a new push certificate, even if the current one is active for 30 more days (PEM_FORCE)",
  )
  .action(pem);
...
program
  ...
  .description(
    'The best way to manage your TestFlight testers and builds from your terminal. Default command is "upload"',
  )
  .option("-c, --config [value]", "Use another config file found in the root")
  .option(
    "[add]",
...
program
  ...
  .option(
    "--skip_upload_graphic_assets",
    "Whether to skip uploading all types of images, including screenshots",
  )
  .option("--skip_build", "Skip build of APK")
  .option("--bump_build_version", "Automatically bump Android build version")
  .option(
    "-a, --track [value]",
    "The Track to upload the Application to: production, beta, alpha or rollout",
  )
  .option("-r, --rollout [value]", "The percentage of the rollout")
  .option("--aab", "Send the AAB instead of the APK")
  .option(
    "--validate_only",
    "Whether to upload and validate but not publish immediately",
  )
  .action(playsend);
...
if (
  ...
  typeof program.args[program.args.length - 1] === "string"
) {
